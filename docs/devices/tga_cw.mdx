...
We have integrated the TGA device from CW in Chemotion ELN. This protocol allows remote access to the PC running the Opus software and the transfer of experiment data to the Chemotion ELN’s inbox.
...
## Target Points
 ...
 by the installation of the software, TightVNC. It works in a similar way as TeamViewer. Users can access the computer remotely through Chemotion-ELN.
  ...
  - **Data Transfer:** Whenever the experiment is completed and a new data file is generated, we have to first rename the file according to the naming convention [below](/docs/devices/ir_vertex_cw#file-naming-convention-details) and then transfer the file to a specific folder. Afterwards the file is transferred to ELN automatically.
...
- While saving the file at the local location in the computer, the user has to name the file following the [naming convention](/docs/devices/ir_vertex_cw#file-naming-convention-details). In this case the local location is **C:\IR Data**.
