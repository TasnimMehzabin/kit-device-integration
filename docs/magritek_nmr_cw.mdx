...
## Target Points
 ...
 by the installation of the software, TightVNC. It works in a similar way as TeamViewer. Users can access the computer remotely through Chemotion-ELN.
  ...
  - **Data Transfer:** Whenever the experiment is completed and a new data file is generated, we have to first rename the file according to the naming convention [below](/docs/magritek_nmr_cw#sample-naming-convention-details) and then transfer the file to a specific folder. Afterwards the file is transferred to ELN automatically.
...
- The next point is regarding the naming of the sample. We have to make sure that the sample name follows the name conventions [rules](/docs/magritek_nmr_cw#sample-naming-convention-details).
